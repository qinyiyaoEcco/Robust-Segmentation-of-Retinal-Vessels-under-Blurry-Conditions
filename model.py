...
class DoubleConv(nn.Module):
    """(Conv => BN => ReLU) * 2 with Channel Attention"""
...
class Down(nn.Module):
    """Downsampling layer with maxpool then double conv"""
...
class Up(nn.Module):
    """Upsampling layer with double conv"""
    def __init__(self, in_channels, out_channels, bilinear=True, use_attention=True):
        super().__init__()
        # Use bilinear interpolation for upsampling or transposed convolution
        if bilinear:
    ...
    def forward(self, x1, x2):
        x1 = self.up(x1)
        # Input might not have perfect size, so calculate padding
...
class UNetWithAttention(nn.Module):
    """UNet model with Channel Attention and Multi-task learning capabilities"""
    def __init__(self, n_channels, n_classes, bilinear=True, enable_attention=True):
        ...
        self.inc = DoubleConv(n_channels, 64, use_attention=enable_attention)
        self.down1 = Down(64, 128, use_attention=enable_attention)
        self.down2 = Down(128, 256, use_attention=enable_attention)
        self.down3 = Down(256, 512, use_attention=enable_attention)
        factor = 2 if bilinear else 1
        self.down4 = Down(512, 1024 // factor, use_attention=enable_attention)
        
        self.up1 = Up(1024, 512 // factor, bilinear, use_attention=enable_attention)
        self.up2 = Up(512, 256 // factor, bilinear, use_attention=enable_attention)
        self.up3 = Up(256, 128 // factor, bilinear, use_attention=enable_attention)
        self.up4 = Up(128, 64, bilinear, use_attention=enable_attention)
        
        # Main segmentation output
        self.outc = OutConv(64, n_classes)
        
        # Edge detection branch (auxiliary task)
        ...
        # Distance transform branch (auxiliary task)
    ...
    def forward(self, x):
        ...
        # Ensure consistent output format in both training and evaluation modes
        # Return multi-task outputs as a tuple in both training and evaluation modes
        # instead of only returning segmentation output in evaluation mode
        return (segmentation, edge, distance) 